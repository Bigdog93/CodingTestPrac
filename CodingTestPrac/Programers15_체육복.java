...
예를 들어, 4번 학생은 3번 학생이나 5번 학생에게만 체육복을 빌려줄 수 있습니다. 체육복이 없으면 수업을 들을 수 없기 때문에 체육복을 적절히
...
체육복을 빌려줄 수 없습니다.
    ...
    3	[3]	        [1]         2
*/
public class Programers15_체육복 {
    public static void main(String[] args) {
        Solution sol = new Solution();
        int[] input_n = {5, 5, 3};
        int[][] input_lost = {{2, 4}, {2, 4}, {3}};
        int[][] input_reserve = {{1, 3, 5}, {3}, {1}};
        int[] result = new int[input_n.length];
        for(int i = 0; i < input_n.length; i++) {
            result[i] = sol.solution(input_n[i], input_lost[i], input_reserve[i]);
    ...
    static class Solution {
        public int solution(int n, int[] lost, int[] reserve) {
            int answer = 0;
            int[] getter = new int[n];
            Arrays.fill(getter, 1);
