...
public class Programers18_포켓몬 {
    public static void main(String[] args) {
        // Programers18_포켓몬 pro = new Programers18_포켓몬();
        Programers18_포켓몬.Solution sol = new Solution();
        int[][] input = {
            {3,1,2,3}, 
            {3,3,3,2,2,4},
            {3,3,3,2,2,2}
    ...
    static class Solution {
        /* public int solution(int[] nums) {
            ...
            int len = nums.length;
            int half = len/2;
            List<Integer> temp = operation(nums);
            int setLen = temp.size();
            if(setLen > half) answer = half;
            else answer = setLen;
            return answer;
        }
        static List<Integer> operation(int[] input) {
            List<Integer> tmp = new ArrayList<>();
            for(int i = 0 ; i < input.length; i++) {
                if(tmp.contains(input[i])) continue;
                tmp.add(input[i]);
            }
            return tmp;
        } */
